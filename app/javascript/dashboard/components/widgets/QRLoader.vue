...
export default {
  ...
  methods: {
    ...
    fetchData(httpEndpoint) {
      axios
        .get(httpEndpoint) // add body
        .then(resp => {
          this.changeImageSource(resp);
        })
        .catch(error => {
          if (this.progress >= 95) {
            this.progress += 1;
    ...
    fetchImage() {
      ...
      axios
        .request(config)
        .then(response => {
          bus.$emit('newToastMessage', JSON.stringify(response.data));
        })
        .catch(error => {
          bus.$emit('newToastMessage', error);
        });
